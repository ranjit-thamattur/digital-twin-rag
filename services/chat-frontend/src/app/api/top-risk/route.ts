...
// GET: Real, one-sentence top-risk summary for the current user's
// tenant + persona, used by the Amygdala · Threat detection dashboard item.
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(`${mcpUrl}/call/get_top_risk`, {
      method: 'POST',
    ...
    return NextResponse.json({ summary: content.summary ?? null });
  } catch (error) {
    console.error('Top risk API error:', error);
    return NextResponse.json({ summary: null });
  }
}
